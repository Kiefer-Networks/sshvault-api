...
New, existing, and soft-deleted addresses receive the same response. Registration returns no account identifiers or access/refresh tokens. New accounts must follow the verification email before login. Correct credentials on a new unverified account, or a token presented to a protected route for that account, return HTTP `403` with `{"error":"verification_required"}`. Each valid signup attempt performs the same expensive password hashing before account-existence checks. New unverified accounts have no installed usable password: their verification token contains only the hash of the password submitted for that specific attempt, and verification atomically installs that hash. A later signup attempt supersedes earlier signup links; verified accounts keep their existing password. Verification links expire after 24 hours.

Verification, recovery, and email-change delivery each have a database-enforced 60-second cooldown per normalized recipient and purpose, shared across clients, IPs, and server processes. Only a recipient digest is stored in the budget table. At most one unused token exists per account and purpose. A throttled signup keeps the same opaque response and invalidates earlier signup links; wait one minute and retry to receive a fresh link. This prevents an earlier attacker-created password from being activated through a stale email after your signup attempt.
