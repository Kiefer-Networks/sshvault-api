...
func (r *pgDeviceRepo) GetByUserID(ctx context.Context, userID uuid.UUID) ([]model.Device, error) {
	query := `
		SELECT id, user_id, name, platform, last_sync, last_seen, created_at
	...
	for rows.Next() {
		var d model.Device
		if err := rows.Scan(&d.ID, &d.UserID, &d.Name, &d.Platform, &d.LastSync, &d.LastSeen, &d.CreatedAt); err != nil {
...
func (r *pgDeviceRepo) UpdateLastSync(ctx context.Context, id, userID uuid.UUID) error {
	now := time.Now()
	query := `UPDATE devices SET last_sync = $1, last_seen = $2 WHERE id = $3 AND user_id = $4`
	_, err := r.pool.Exec(ctx, query, now, now, id, userID)
	if err != nil {
